...
class MOE(object):
    ...
    def model_train(self, X, y, x_test, y_test, X_val, y_val, i):
        logging.info("Expert {}".format(X.shape[0]))
        model = nn_models()
        model.ip_shape = X.shape
        model = model.lenet5()
        model.learning_rate = 0.001
        model.fit(X, y, batch_size=128, epochs=self.epoch, validation_data=(X_val, y_val),
    ...
    def main(self, x_train, y_train, x_test, y_test, x_val, y_val):
        ...
        model_p.learning_rate = 0.001
        model_p = model_p.lenet5()
        model_p.fit(x_train, y_train, batch_size=128, epochs=self.epoch, validation_data=(x_val, y_val),
                    verbose=1, callbacks=[self.early_stopping])
        ...
        logging.info("Whole {}".format(x_train.shape[0]))
        ...
        for i in range(self.iters):
            ...
            for j in range(self.experts):
                ...
                yhat_train = model.predict(x_train, batch_size=128)
                yhats_test = model.predict(x_test, batch_size=128)
                yhats_val = model.predict(x_val, batch_size=128)
            ...
            yhats_train = model.predict([prime_op_tr, yhat_tr], batch_size=128)
            yhats_test = model.predict([prime_op_tt, yhat_tt], batch_size=128)
            yhats_val = model.predict([prime_op_v, yhat_val], batch_size=128)
...
if __name__ == '__main__':
    ...
    logging.basicConfig(filename='../log/cifar_subset.' + str(start_time), level=logging.INFO)
